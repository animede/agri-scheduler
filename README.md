...
- **AI画像解析（任意機能）**: 種苗会社の種袋画像をアップロードすると、Claude Vision APIが
  栽培暦を自動抽出し、品種登録フォームに反映する（要 `ANTHROPIC_API_KEY`。未設定でも他の
  機能はすべて通常どおり使用できる）。
...
`ANTHROPIC_API_KEY` が未設定、またはネットワーク未接続の場合、品種登録フォームの「AIで解析」は
